...
import Board, { type PlayMode } from '@/components/board';
...
function App() {
  ...
  const handleApplySeed = () => {
    ...
    setGame(newGame);
    setGrid(newGame.board.map((row) => row.map((cell) => (cell ? 'filled' : 'empty'))));
  };
      ...
      <div className="mt-4 flex flex-col gap-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            ...
            <Input id="seed" type="text" value={seed} onChange={handleSeedChange} className="w-32" placeholder="Seed" />
          </div>
          <Button onClick={handleApplySeed}>Generate</Button>
        </div>
      </div>
      <div className="mt-4">
